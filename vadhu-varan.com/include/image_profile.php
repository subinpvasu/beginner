...
if ($result === false) {
    echo '<br>An error occurred during file processing.';
		...
		$new_file_name =  $result[1];
				...
				echo '<p style="font-weight:bold;color:#C4160F;text-align:center;">Successfully  Uploaded Your Profile Image.</p><BR/>
...
xmlhttp.open("GET","validation.php?msg=truephoto&data="+str+"&auth="+sbn+"&idt="+idt,true);
xmlhttp.send();
}
function showDetail()
{
var z = "'.$id.'";
if(z.match("img")){
var n=z.replace("pro","check");
}
else
{
var n=z.replace("side","check");
}
this.window.opener.document.getElementById(z).innerHTML=da;
this.window.opener.document.getElementById(n).style.visibility="visible";
setTimeout(function(){this.window.opener.document.getElementById(n+"s").click();},1000);
setTimeout(function(){this.window.opener.document.getElementById("othersubmit").click();},2000);
}
function countDown(){ 
var status = " ' . $check . ' "; 
if (count <=0){  
...
if ($check != 'true') {
	?>
<body>
<form action="image_profile.php" method="post" onsubmit=""
